...
export async function sendOrderNotification(orderData) {
  ...
  try {
    ...
    // 4. Fetch Subscriptions from MongoDB (if connected)
    let mongoSubs = []
    if (mongoConnected) {
      try {
    ...
    // 8. Dispatch Telegram Order Notification concurrently
    try {
      const { sendTelegramOrderNotification } = await import('./telegramNotification')
      sendTelegramOrderNotification(orderData).catch(err => console.warn('[OrderNotification] Telegram notice:', err.message))
    } catch (tgErr) {
